...
import ff.choices
import ff.jobs.job_manager
import ff.jobs.job_store
from ff import config, content_analyser, face_classifier, face_detector, face_landmarker, face_masker, face_recognizer, inference_manager, logger, state_manager, translator, video_manager
from ff.common_helper import get_first, is_macos
from ff.download import conditional_download_hashes, conditional_download_sources, resolve_download_url
from ff.execution import has_execution_provider
from ff.face_analyser import get_average_face, get_many_faces, get_one_face, scale_face
from ff.face_helper import paste_back, warp_face_by_face_landmark_5
from ff.face_masker import create_area_mask, create_box_mask, create_occlusion_mask, create_region_mask
from ff.face_selector import select_faces, sort_faces_by_order
from ff.filesystem import filter_image_paths, has_image, in_directory, is_image, is_video, resolve_relative_path, same_file_extension
from ff.model_helper import get_static_model_initializer
from ff.processors.modules.face_swapper import choices as face_swapper_choices
from ff.processors.modules.face_swapper.types import FaceSwapperInputs
from ff.processors.pixel_boost import explode_pixel_boost, implode_pixel_boost
from ff.processors.types import ProcessorOutputs
from ff.program_helper import find_argument_group
from ff.thread_helper import conditional_thread_semaphore
from ff.types import ApplyStateItem, Args, DownloadScope, Embedding, Face, InferencePool, ModelOptions, ModelSet, ProcessMode, VisionFrame
from ff.vision import read_static_image, read_static_images, read_static_video_frame, unpack_resolution
...
def register_args(program : ArgumentParser) -> None:
	...
	if group_processors:
		...
		ff.jobs.job_store.register_step_keys([ 'face_swapper_model', 'face_swapper_pixel_boost', 'face_swapper_weight' ])
...
def forward_swap_face(source_face : Face, target_face : Face, crop_vision_frame : VisionFrame) -> VisionFrame:
	...
	if is_macos() and has_execution_provider('coreml') and model_type in [ 'ghost', 'uniface' ]:
		face_swapper.set_providers([ ff.choices.execution_provider_set.get('cpu') ])
